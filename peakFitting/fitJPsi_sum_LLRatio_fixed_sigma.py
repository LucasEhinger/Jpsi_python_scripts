...
x_data,y_data,yerr_data=getXY(infiles=[filepath+tree for tree in dataFiles],
                              weights=[1,1,1],histname=histname,rebin=rebin_binned)
dx = x_data[1]-x_data[0]

def gaus_bdk_exp(x,a0,a1,A,mu):
    return (a0*10**6*np.exp(x*a1) + A * norm.pdf(x,loc=mu,scale=sigma_fixed))

def integrated_gaus_bkd_exp(x,a0,a1,A,mu):
    df=[]
    for xval in x:
        df_val = integrate.quad(gaus_bdk_exp, xval - dx / 2, xval + dx / 2, args=(a0, a1, A, mu),epsrel=0.01)
        df.append(df_val[0])
    return df
# </editor-fold>
...
def gaus_exp_bdk_pdf(x,a0,a1,mu):
    pdf_val=(1 / np.sqrt(2 * np.pi * sigma_fixed ** 2) * np.exp(-(x - mu) ** 2 / (2 * sigma_fixed ** 2)) + a0 *a1* np.exp(a1*x)/(np.exp(x_fit_max*a1)-np.exp(x_fit_min*a1))) / (1 + a0)
    return pdf_val
...
def minus_log_likelihood(params):
    ...
    A=abs(A)
    a0=abs(a0)
    tmp=w_fit*np.log(A*gaus_exp_bdk_pdf(x_fit,a0,a1,mu))
    return A-tmp.sum()
...
initial_guess = [popt[0]/(popt[2]*popt[1])*(np.exp(popt[1]*x_fit_max)-np.exp(popt[1]*x_fit_min)),popt[1],10,3.05]
...
a0, a1, Aval, mu = popt
tmp = w_fit * np.log(gaus_exp_bdk_pdf(x_fit, a0, a1, mu))
sig_sum = tmp.sum()
...
plt.savefig(f"../../files/figs/peakFits/subthreshold/Mee_{A}_ratio_subt_fixed_sigma_noTrackShower_{vers}.pdf")
plt.show()