...
xmin=0.8
xmax=1.2
mu=np.zeros(len(theta_bins)-1)
sigma=np.zeros(len(theta_bins)-1)
mu_err=np.zeros(len(theta_bins)-1)
sigma_err=np.zeros(len(theta_bins)-1)
...
for i in range(len(theta_bins) - 1):
    ...
    for j in range(len(p_bins) - 1):
        histname = f"PoverE_{theta_bins[i]}_{theta_bins[i + 1]}_{p_bins[j]}_{p_bins[j + 1]}"
